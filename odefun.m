function dYdt = odefun(t, Y, Ms, m, Mu_d, g, lambda, rr, L, omega)
    theta = Y(1);
    dtheta = Y(2);
    y = Y(3);
    dy = Y(4);
    ...
    % Second equation
    d2y = (Mu_d*m*g*cos(theta)*exp(Mu_d*(pi+theta)) + Mu_d*m*l*omega^2*exp(Mu_d*(pi+theta)) - Mu_d*Ms*g) / ...
          (Ms*Mu_d + lambda*rr*exp(Mu_d*(pi+theta)) - lambda*rr + m*Mu_d*exp(Mu_d*(pi+theta)));
